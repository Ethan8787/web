...
export default function Countdown() {
    ...
    useEffect(() => {
        ...
        const update = () => {
            ...
            setDays({
                exam: d1 > 0 ? Math.ceil(d1 / 86400000) : 0,
                grad: d2 > 0 ? Math.ceil(d2 / 86400000) : 0
            });
        };
        update();
    ...
    const handleCapture = async (ref, prefix) => {
        if (localStorage.getItem('verified') !== 'true') {
            ...
            try {
                ...
                if (id.trim() === storedUUID) localStorage.setItem('verified', 'true');
                else return alert("Authentication Failed.");
            } catch (e) {
            ...
            const canvas = await html2canvas(ref.current, {
                ...
                onclone: (clonedDoc) => {
                    ...
                    if (card) {
                        ...
                        wm.innerText = '@Ethan';
                        wm.style.cssText = `position:absolute;top:0;left:0;width:100%;height:100%;display:flex;align-items:center;justify-content:center;opacity:0.05;font-size:0.8rem;color:white;pointer-events:none;z-index:9999;font-family:sans-serif;`;
                        card.appendChild(wm);
                    }
                }
            });
    ...
    return (
        <div className="countdown-main-layout">
            <div className="side-tools">
                ...
                <button onClick={() => handleCapture(refs.grad, 'Grad')} className="tool-btn" title="截圖畢業倒數" style={{marginTop: '10px'}}>
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#1a1a1a" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
