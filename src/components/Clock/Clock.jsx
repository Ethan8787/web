...
export default function Clock() {
    ...
    const animate = () => {
        if (mode === "CLOCK") {
            ...
            const currentElapsed = isRunning ? Date.now() - startTimeRef.current + elapsedRef.current : elapsedRef.current;
    ...
    return (<div className={`clock-shell ${!showControls ? "hide-cursor" : ""}`}>
        <Background/>
        ...
        <div
            className={`content-layout ${laps.length > 0 && mode === "STOPWATCH" ? "split-view" : "centered-view"}`}>
            <div
                ref={displayRef}
            ...
            {mode === "STOPWATCH" && laps.length > 0 && (
                <div className={`laps-container ${showControls ? "visible" : "dimmed"}`}>
                    {laps.map((lap) => (<div key={lap.id} className="lap-item">
        ...
        <div className={`ui-overlay ${showControls ? "visible" : "hidden"}`}>
            <button className="btn-fs" onClick={toggleFullscreen}>
                {isFullscreen ? (<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                      strokeWidth="1.5">
                    <path
            ...
            <div className="mode-switch-container">
                <button className={`mode-btn ${mode === "CLOCK" ? "active" : ""}`}
                        onClick={() => setMode("CLOCK")}>CLOCK
                </button>
                <div className="divider"></div>
                <button className={`mode-btn ${mode === "STOPWATCH" ? "active" : ""}`}
                        onClick={() => setMode("STOPWATCH")}>STOPWATCH
                </button>
            </div>
        </div>
    </div>);
